  ...
  return (
    <div className="py-6 sm:py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 text-xs text-electric-pink font-semibold uppercase tracking-wider mb-2.5">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold whitespace-nowrap transition-all duration-200 cursor-pointer ${
                activeCategory === cat.id
                  ? 'bg-electric-pink text-white shadow-md shadow-electric-pink/30'
                  : theme === 'dark'
                  ? 'bg-slate-900 text-slate-300 hover:bg-slate-800 border border-slate-800'
                  : 'bg-white text-slate-700 hover:bg-primary-container/20 border border-primary-container/40'
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className={`p-4 rounded-2xl border mb-8 flex flex-wrap items-center justify-between gap-4 ${
          theme === 'dark' ? 'bg-slate-900/60 border-slate-800' : 'bg-white/90 border-primary-container/40 shadow-sm'
        }`}>
          {/* Left: Filter Toggle & Results Count */}
          <div className="flex items-center gap-4">
            <button
              onClick={() => setIsFilterDrawerOpen(!isFilterDrawerOpen)}
              className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-primary-container/20 dark:bg-slate-800 text-tertiary dark:text-baby-pink text-xs font-bold hover:bg-primary-container/40 transition-colors cursor-pointer"
            >
              ...
              <select
                ...
                className={`pl-3 pr-8 py-1.5 text-xs font-bold rounded-xl border outline-hidden transition-colors cursor-pointer appearance-none ${
                  theme === 'dark'
                    ...
                    : 'bg-primary-container/15/40 border-baby-pink text-slate-800'
                }`}
              >
        ...
        {isFilterDrawerOpen && (
          <motion.div
            ...
            className={`p-5 rounded-2xl border mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 ${
              theme === 'dark' ? 'bg-slate-900 border-slate-800 text-white' : 'bg-primary-container/20/30 border-primary-container/40 text-slate-800'
            }`}
          >
            {/* Price Filter */}
            <div>
              <div className="flex justify-between items-center text-xs font-bold mb-2">
                ...
                <span className="text-electric-pink font-black">฿0 - ฿{maxPrice.toLocaleString()}</span>
              </div>
              <input
                type="range"
                min={200}
                ...
                className="w-full accent-electric-pink cursor-pointer"
              />
            </div>
            ...
            <div className="flex items-center">
              <label className="flex items-center gap-2.5 text-xs font-bold cursor-pointer select-none">
                <input
                  ...
                  className="w-4 h-4 rounded text-electric-pink accent-electric-pink focus:ring-electric-pink"
                />
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-xs font-bold border transition-colors ${
                    selectedSizeFilter === 'all'
                      ? 'bg-electric-pink text-white border-electric-pink'
            ...
            <div className="flex items-end justify-start sm:justify-end">
              <button
                ...
                className="px-4 py-2 rounded-xl text-xs font-bold text-slate-500 hover:text-tertiary flex items-center gap-1.5 border border-slate-200 dark:border-slate-700 hover:border-baby-pink cursor-pointer"
              >
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20 bg-primary-container/15/20 dark:bg-slate-900/30 rounded-3xl border border-dashed border-baby-pink dark:border-slate-800 p-8 space-y-4">
            <div className="w-16 h-16 rounded-full bg-primary-container/40 dark:bg-pink-950 flex items-center justify-center mx-auto text-electric-pink">
            ...
            <button
              onClick={resetFilters}
              className="px-5 py-2.5 rounded-xl bg-electric-pink text-white text-xs font-bold shadow-md hover:bg-tertiary transition-colors"
            >
