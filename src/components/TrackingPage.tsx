...
export const TrackingPage: React.FC = () => {
  ...
  // Simplified 4-stage stepper to match the "Track Your Sparkle" template
  const steps = [
  ...
  return (
    <div className={`relative overflow-hidden py-10 sm:py-14 ${theme === 'dark' ? 'bg-slate-950' : 'bg-gradient-to-b from-primary-container/20 via-primary-container/15/40 to-transparent'}`}>

      {/* Decorative Y2K sparkles (desktop only) */}
      <div className="hidden md:block absolute top-10 left-10 text-baby-pink text-xl select-none">★</div>
      <div className="hidden md:block absolute bottom-16 right-16 text-baby-pink text-2xl select-none">✦</div>
      <div className="hidden md:block absolute top-1/3 left-6 text-baby-pink text-lg select-none">♡</div>
      <div className="hidden md:block absolute bottom-10 right-10 text-baby-pink text-xl select-none">✦</div>

      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 space-y-8">

        {/* Brand Logo */}
        <div className="flex justify-center">
          <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full overflow-hidden shadow-lg border-4 border-baby-pink bg-white flex items-center justify-center">
            <span className="font-serif-luxury text-xl sm:text-2xl font-bold text-electric-pink text-center leading-tight italic">
              Cozy<br />Select
            </span>
          </div>
        </div>
        ...
        <div className="text-center space-y-2">
          <h1 className="text-2xl sm:text-4xl font-serif-luxury font-bold text-slate-900 dark:text-white flex items-center justify-center gap-2">
            <span>{language === 'th' ? 'ติดตามความสปาร์กของคุณ' : 'Track Your Sparkle'}</span>
            <Sparkles className="w-6 h-6 text-electric-pink" />
          </h1>
        ...
        <div className="w-full max-w-md mx-auto">
          <form onSubmit={(e) => { e.preventDefault(); handleSearch(searchInput); }} className="relative">
            <input
              ...
              className="w-full pl-6 pr-32 py-4 rounded-full border-2 border-baby-pink dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:border-electric-pink font-medium text-sm placeholder-slate-400 shadow-[0_4px_20px_rgba(255,193,204,0.35)] transition-all"
            />
            <button
              type="submit"
              disabled={isSearching}
              className="absolute right-2 top-2 bottom-2 px-6 rounded-full bg-slate-900 dark:bg-electric-pink text-white text-xs font-bold uppercase tracking-wide hover:bg-tertiary transition-colors shadow-md cursor-pointer"
            >
          ...
          <div className="mt-3 flex items-center justify-center gap-2 text-xs text-slate-400">
            ...
            <button
              ...
              className="font-mono text-electric-pink hover:underline"
            >
              COZY-882101
            </button>
            <span>•</span>
          ...
          {searchError && (
            <div className="mt-3 p-3 rounded-xl bg-primary-container/15 dark:bg-rose-950/40 text-tertiary text-xs font-semibold flex items-center gap-2 justify-center">
        ...
        {foundOrder && (
          <motion.div
            ...
            className={`relative overflow-hidden rounded-3xl p-6 sm:p-8 border space-y-8 ${
              theme === 'dark' ? 'bg-slate-900/90 border-slate-800 text-white' : 'bg-white/95 backdrop-blur-md border-primary-container/40 shadow-xl shadow-primary-container/40/40 text-slate-800'
            }`}
          >
            {/* Watermark truck icon */}
            <Truck className="w-20 h-20 text-primary-container/40 dark:text-slate-800 absolute top-4 right-4 pointer-events-none" />
            ...
            <div className="relative w-full">
              ...
              <div
                className="absolute top-4 sm:top-5 left-0 h-1 bg-baby-pink rounded-full shadow-[0_0_10px_rgba(244,114,182,0.6)] transition-all duration-500"
                  ...
                  return (
                    <div key={st.key} className="flex flex-col items-center gap-2 text-center w-1/4">
                      <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-900 shadow-md transition-all ${
                        isCurrent
                          ? 'bg-electric-pink text-white animate-pulse shadow-electric-pink/60'
                          : isDone
                      ...
                      <span className={`text-[10px] sm:text-xs font-bold uppercase tracking-wide ${
                        isCurrent ? 'text-tertiary' : isDone ? 'text-slate-800 dark:text-slate-200' : 'text-slate-400'
                      }`}>
            ...
            {/* Courier Details + Map */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-primary-container/20/40 dark:bg-slate-800/50 rounded-2xl p-6 border border-primary-container/40 dark:border-slate-700">

              <div className="space-y-4">
                <h3 className="text-xs font-bold uppercase tracking-widest text-slate-700 dark:text-slate-300 border-b border-primary-container/40 dark:border-slate-700 pb-2">
                ...
                <div>
                  ...
                  <p className="text-xl sm:text-2xl font-black text-tertiary">{estimatedDelivery}</p>
                </div>
                <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-3 rounded-full shadow-sm border border-primary-container/40 dark:border-slate-700">
                  <div className="w-10 h-10 bg-primary-container/40 dark:bg-pink-950 rounded-full flex items-center justify-center text-tertiary">
              ...
              <div className="rounded-2xl overflow-hidden h-40 sm:h-44 bg-gradient-to-br from-primary-container/40 to-primary-container/30 dark:from-slate-800 dark:to-slate-900 relative flex items-center justify-center border border-primary-container/40 dark:border-slate-700">
                <MapPin className="w-16 h-16 text-baby-pink dark:text-slate-700 absolute opacity-50" />
                <div className="z-10 bg-white/90 dark:bg-slate-900/90 px-4 py-2 rounded-full text-xs font-bold text-tertiary shadow-md flex items-center gap-2">
            ...
            {/* Journey Checkpoints */}
            <div className="pt-2 border-t border-primary-container/40 dark:border-slate-800">
              ...
              <div className="space-y-4 pl-3 border-l-2 border-baby-pink dark:border-slate-700 ml-3">
                {mockTimelineEvents.map((evt, i) => (
                  <div key={i} className="relative pl-4 space-y-0.5">
                    <div className={`w-3 h-3 rounded-full absolute -left-[19px] top-1 border-2 border-white dark:border-slate-900 ${
                      i === 0 ? 'bg-electric-pink' : 'bg-slate-300 dark:bg-slate-600'
                    }`} />
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs gap-0.5">
                      ...
                      <span className="text-[11px] text-electric-pink font-semibold">{evt.time}</span>
                    </div>
            ...
            {/* Ordered Items */}
            <div className="pt-4 border-t border-primary-container/40 dark:border-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {foundOrder.items.map((it: any) => (
                  <div key={it.id} className="p-2.5 rounded-xl bg-primary-container/20/40 dark:bg-slate-800/40 border border-primary-container/40 dark:border-slate-800 flex items-center gap-3">
                    <img
                    ...
                    <div className="text-xs min-w-0 flex-1">
                      ...
                      <p className="text-tertiary font-bold">฿{(it.unitPrice * it.quantity).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
              </div>
